...
public class CryptoChatClientServer extends Thread implements Observer {
	...
	public CryptoChatClientServer(Socket clientSocket, Dashboard dashboard, CryptoModule cryptoModule) {
...
//			}
	...
	public void run() {
		...
		try {
			...
			helloMessage = cryptoModule.read(input);

			this.setName(helloMessage.getSender());
		} catch (IOException e) {
			logger.error("Problems during hello message reading: " + e);
			e.printStackTrace();
		}
		...
		dashboard.deleteObserver(this);
		logger.debug("Server for " + this.getName() + " terminated");
	}
